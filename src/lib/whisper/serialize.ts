import type { MessageRow, Inbox as InboxRow } from "@/lib/db/schema/whisper";
import type { MessageStatus, WhisperMessage, ReceiptView, PublicEntry } from "@/lib/whisper/types";

/** Owner-facing message DTO (safe to send the owner). */
export function toOwnerMessage(row: MessageRow): WhisperMessage {
  return {
    id: row.id,
    body: row.body,
    reply: row.reply,
...
export function toReceiptView(row: MessageRow): ReceiptView {
  return {
    id: row.id,
    body: row.body,
    reply: row.reply,
